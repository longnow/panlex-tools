...
from nltk import PerceptronTagger, RegexpParser, TreebankWordTokenizer
from nltk.corpus import brown
from nltk.tag import UnigramTagger, BigramTagger, TrigramTagger, DefaultTagger
from nltk.tree import Tree
from gary.source import ignore_parens, append_synonym, DcsMapper, pretag_ex, pretag_df
...
def extract_article(text, pos):
    if not re.search('\m(a|an|the)\M', text):
        return text,pos

    fields = text.split('‣')

    for i in range(len(fields)):
        match = re.search('^(?:a|an|the)\s+(\w+)', fields[i])

        if match:
            if match[1].lower() not in ['lot', 'little', 'priori', 'posteriori', 'bit']:
                fields[i] = re.sub('^(?:a|an|the)\s+', '', fields[i])
                field_pos = mapper[ append_synonym(pos, 'noun')]
                fields[i] = pretag_ex('%s%s' % (fields[i],field_pos))
...
def extract_inf_to(text):
    global tokenizer,tagger
    if not re.search('\mto\M', text) or text == 'to':
        return text
    ...
    for i in range(len(fields)):
        if re.search('^to be ', fields[i]):
            fields[i] = re.sub('^to be ', '', fields[i])
            continue

        if fields[i] == 'to':
            continue

        to_be_match = re.search('^to be ', fields[i])

        if to_be_match:
            new_pos = mapper['adj']
            results.append('⫷ex⫸%s%s' % (fields[i],new_pos))

        selected = False
        tokens = tokenizer.tokenize(fields[i])
        ...
        if tagged_tokens[0][1] == 'TO' and tagged_tokens[1][1] != 'DT':
            selected = True
            if not re.search("(everybody|somebody|which|this|that)", tagged_tokens[1][0]):
                if tagged_tokens[1][0] not in exclude_list:
                    new_pos = mapper['verb']
                    new_text = re.sub('^to\s+', '',fields[i])
                    fields[i] = '⫷ex⫸%s%s' % (new_text,new_pos)

        if not selected and len(fields[i].strip()) > 0:
            fields[i] = '⫷ex⫸%s' % fields[i]
