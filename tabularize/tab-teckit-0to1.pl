...
open SEG2OLD, '>:encoding(utf8)', ("$fnbase-seg2-$ver.txt");
...
`txtconv -t secondary/kantipur.tec -i "$fnbase-seg2-$ver.txt" -o "$fnbase-seg2-$newver.txt"`;
# Recode the recoding input file.

open DICIN, '<:encoding(utf8)', "$fnbase-$ver.txt";
# Open the input file again for reading.

open SEG2NEW, '<:encoding(utf8)', "$fnbase-seg2-$newver.txt";
